	...
	return (
		<div className="navbar bg-base-100 border-b-2 border-base-300">
			<div className="navbar-start">
							...
							<ul className="menu p-4 w-60 min-h-full bg-base-200 text-base-content z-50">
								{/* Sidebar content here */}
								{
									token.role === 'User'
										?
										<>
											<li><Link to={'User/Cart'} className="text-xl" onClick={Toggle}><AiOutlineShoppingCart size={18} />Cart</Link></li>
											<li><Link to={'User/Orders'} className="text-xl" onClick={Toggle}><BsMinecartLoaded size={18} />Orders</Link></li>
										</>
										:
										token.role === 'Shop'
											&&
											<>
												<li><Link to={'Shop/Inventory'} className="text-xl" onClick={Toggle}><MdOutlineInventory2 size={18} />Inventory</Link></li>
												<li><Link to={'Shop/Orders'} className="text-xl" onClick={Toggle}><TbShoppingCartCog size={18} />Orders</Link></li>
											</>
								}
							</ul>
						</div>
				...
				<Link to={'/'} className="btn btn-ghost normal-case text-xl">Shop Locator<img className="h-full mix-blend-multiply" src={Logo} alt="logo" /></Link>
			</div>
				...
				<ul className="menu menu-horizontal px-1">
					{/* Desktop content here */}
					{
						token.role === 'User'
							?
							<>
								<li><Link to={'User/Cart'} className="text-xl">Cart</Link></li>
								<li><Link to={'User/Orders'} className="text-xl">Orders</Link></li>
							</>
							:
							token.role === 'Shop'
								?
				...
				</label>
				{
					token.token !== null
						...
						<div className="flex justify-center items-center gap-5">
							<Link to={`${token.role === 'Shop' ? 'Shop' : token.role === 'User' && 'User'}/Account`}>
